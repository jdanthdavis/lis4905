...
def data_manipulation():
    ...
    print("\n26. Frequency of all (values_counts()) in a column (here, 'genre') returns 1st 10:")
    dataseries = df['genre'].squeeze()
    print(dataseries.value_counts().head(10))
    print("\n27. Relationships between variables using .corr() method:")
    print(df.corr())
    print("\n28. Conditional selections.")
    print("\n\t28a. Display oldest and newest release years for movies is dataset:")
    max_y = df["year"].max()
    min_y = df["year"].min()
    print(f"Oldest: {max_y}, Newest: {min_y}")
    
    print("\n\t28b. Filter by one director, return 3 records:")
    print(df[df['director']=="Clint Eastwood"].head(3))
    print("\n\t28c. Filter by rating (>=8.0), return 3 records:")
    print(df[df['rating']>=8.0].head(3))
    print("\n\t28d. Filter by two directors (using 'or'). Logical operators | for 'or' & for 'and', return 1st 5 records:")
    print(df[(df['director']=="James Gunn") | (df['director']=="Clint Eastwood")].head(5))
    print("\n\t28e. Filter by same two directories (using 'isin'), return 1st 5 records:")
    directors = ["Clint Eastwood", "James Gunn"]
    print(df[df['director'].isin(directors)].head(5))
    print("\n\t28f. Filter by all movies released between 2005 and 2010, inclusive, having rating above 8.5:")
    print(df[(df['year'] >= 2005) & (df['year'] <= 2010) & (df['rating'] > 8.5)])
    ...
    q75 = df['revenue_millions'].quantile(0.75)
    print(df[(df['year'] >= 2005) & (df['year'] <= 2010) & (df['rating'] > 8.0) & (df['revenue_millions'] > q75) ])
    print("\n29. Crate function, watch_rating(), return 'yes' or 'no' based upon 8.5 or greater rating. \nUse Python's apply() function to pass 'rating' attribute to function,\nand assign return values to new attribute 'watch_movie', display 1st 10 rows.")
    ...
    df['watch_rating'] = df['rating'].apply(func=watch_rating)
    print(df.head(10))
